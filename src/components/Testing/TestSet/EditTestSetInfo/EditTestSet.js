...
import TestCategoryData from "./TestCategoryData";

const EditTestSet = () => {
  const location = useLocation();
  const EditTestSetData = location.state?.EditTestSetData || {};
  const EdittestsetId = EditTestSetData.id;
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.post(
          `https://ifhirehealth.clindcast.com/api/updatetestset/${EdittestsetId}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            ...
            <div className="col-md-9">
              <div className="box mb-2 form-box">
                <div className="box-top-title fhir-title">
                  <span className="pd-overview-count">Edit Test Set </span>
                </div>
                ...
                <TestCategoryData EdittestsetId={EdittestsetId} />
              </div>
            </div>
          </div>
        </div>
