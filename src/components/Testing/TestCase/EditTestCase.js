...
const EditTestCase = () => {
  ...
  const EditTestCaseData = location.state?.EditTestCaseData || {};
  const EdittestcaseId = EditTestCaseData.id;
  ...
  const [formData, setFormData] = useState({
    scenario: EditTestCaseData.scenario || "",
    description: EditTestCaseData.description || "",
    fhir_query: EditTestCaseData.fhir_query || "",
    database_query: EditTestCaseData.database_query || "",
    comments: EditTestCaseData.comments || "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.post(
          `https://ifhirehealth.clindcast.com/api/updatetestcase/${EdittestcaseId}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            ...
            <div className="col-md-9">
              <div className="box mb-2 form-box">
                <div className="box-top-title fhir-title">
                  <span className="pd-overview-count">Edit Test Case </span>
                </div>
                      ...
                      <div className="col-md-6">
                        <div className="mb-3">
                          <label htmlFor="scenario" className="form-label">
                            Scenario <span className="text-danger fs-6">*</span>
                          </label>
                          <input
                            type="text"
