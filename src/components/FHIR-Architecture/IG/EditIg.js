...
const EditIg = () => {
  ...
  const [formData, setFormData] = useState({
    name: EditIgData.name || "",
    link: EditIgData.link || "",
    customer: EditIgData.customer || "",
    comments: EditIgData.comments || "",
    ig_version: EditIgData.ig_version || "",
    attachment: EditIgData.attachment || null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.post(
          `https://ifhirehealth.clindcast.com/api/updateig/${EditigId}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
                      ...
                      >
                        <div className="main-container">
                          <div id="dropZone" className="drop-zone">
                            ...
                            {selectedFiles && (
                            <span className="mleft">{selectedFiles.name}</span>
                          )}
                          </div>
                          {errorMessage && (
